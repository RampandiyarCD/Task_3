document.addEventListener('DOMContentLoaded', function() {
    ...
    if (greetingElement) {
        greetingElement.textContent = `Greetings ${currentUser.name.split(' ')[0]}!`;
    }
    ...
    function updateAccountCards() {
        ...
        if (savingsCard && savingsAccount) {
            savingsCard.querySelector('.text-gray-700.font-medium').textContent = 'Savings Account';
            savingsCard.querySelector('.closing-balance').textContent =
                `Closing balance: ${formatCurrency(savingsAccount.current_balance)}`;
        ...
        if (currentCard && currentAccount) {
            currentCard.querySelector('.text-gray-700.font-medium').textContent = 'Current Account';
            currentCard.querySelector('.current-balance').textContent =
                `Closing balance: ${formatCurrency(currentAccount.current_balance)}`;
        ...
        if (creditCard && creditAccount) {
            creditCard.querySelector('.text-gray-700.font-medium').textContent = 'Credit Account';
            creditCard.querySelector('.credit-balance').textContent =
                `Current balance: ${formatCurrency(creditAccount.current_balance)}`;

            const creditDetails = creditCard.querySelector('.account-details');
            if (creditDetails) {
                creditDetails.querySelector('.credit-name').textContent =
                    currentUser.name;
                creditDetails.querySelector('.credit-card-number').textContent =
                    `${creditAccount.card_number.slice(0, 4)}-${creditAccount.card_number.slice(4, 8)}-${creditAccount.card_number.slice(8, 12)}-${creditAccount.card_number.slice(12)}`;
    ...
    function activateTab(activeTab) {
        ...
        tabs.forEach(tab => {
            tab.classList.remove('text-orange-600', 'border-orange-600', 'bg-orange-50');
        ...
        activeTab.classList.add('text-orange-600', 'border-orange-600', 'bg-orange-50');
    }
    ...
    function showStatement(accountType) {
        ...
        modalTitle.textContent = `${accountType.charAt(0).toUpperCase() + accountType.slice(1)} Account Statement`;
        ...
        account.statement.forEach(transaction => {
            ...
            row.innerHTML = `
                ...
                <td class="px-6 py-4 whitespace-nowrap text-sm ${amountClass} font-medium">${amountSign}${formatCurrency(Math.abs(transaction.amount))}</td>
    ...
    if (transferForm) {
        ...
        if (currentUser.accounts.savings) {
            ...
            option.textContent = `Savings Account (${currentUser.accounts.savings.account_number.slice(-4)})`;
        ...
        if (currentUser.accounts.current) {
            ...
            option.textContent = `Current Account (${currentUser.accounts.current.account_number.slice(-4)})`;
            accountSelect.appendChild(option);
        }
        ...
        dummyBeneficiaries.forEach(beneficiary => {
            ...
            option.textContent = `${beneficiary.name} (${beneficiary.accountNumber.slice(-4)})`;
        ...
        transferForm.addEventListener('submit', function(e) {
            ...
            const selectedBeneficiaryText = beneficiarySelect.options[beneficiarySelect.selectedIndex].textContent;
            alert(`Successfully transferred ${formatCurrency(amount)} to ${selectedBeneficiaryText}!`);
